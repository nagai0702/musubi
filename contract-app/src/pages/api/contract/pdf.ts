...
import { getPdfCache, cacheKey } from '../../../lib/pdf-cache';
...
export const GET: APIRoute = async ({ url }) => {
  ...
  // キャッシュから取得（高速）
  let pdfBuffer = getPdfCache(cacheKey(token, sheetIdx));

  // キャッシュなければAPIから取得
  if (!pdfBuffer) {
    try {
      pdfBuffer = await exportSingleSheetPdf(spreadsheetId, sheetNames[sheetIdx]);
    } catch (e: any) {
